...
<head>
    ...
    <title>Politik Görüs</title>
</head>
<body>
<style>
body {
background-image: url("./foto/policy_image.jpg");
...
$sorguIlk = $baglanti->query("SELECT id from makale order by id asc limit 1");
$sonucIlk = $sorguIlk->fetch_assoc();
$ilkId = $sonucIlk["id"];

$sorguSon = $baglanti->query("SELECT id from makale order by id desc limit 1");
...
if ($_GET) {
    ...
    if ($yon == "ileri") {
        $sorgu = $baglanti->query("SELECT * FROM makale where id > $id limit 1"); // Makale tablosundaki veri çekiyoruz.
    }
    if ($yon == "geri") {
        $sorgu = $baglanti->query("SELECT * FROM makale where id < $id ORDER  by id desc limit 1"); // Makale tablosundaki veri çekiyoruz.
    }


} else {
    //eğer sayfa ilk defa açılmışsa ilk değeri alıyoruz.
    $sorgu = $baglanti->query("SELECT * FROM makale limit 1");
...
<div class="card text-info bg-dark mb-3 ">
<div class="container" >
<div class="card-body"> 
    
    <h1 class="card-title text-center">Politik Görüş </h1>
    <p class="card-text">
...
function yolla(){
window.location.href="politik_gorus.php?yon=ileri&id=<?php echo $sonuc["id"]; ?>"
}
