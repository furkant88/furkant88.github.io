...
<head>
    ...
    <title>Felsefi Görüş</title>
</head>
<body>
<style>
body {
background-image: url("./foto/felsefi_bg.jpg");
...
$sorguIlk = $baglanti->query("SELECT id from makale1 order by id asc limit 1");
$sonucIlk = $sorguIlk->fetch_assoc();
$ilkId = $sonucIlk["id"];

$sorguSon = $baglanti->query("SELECT id from makale1 order by id desc limit 1");
...
if ($_GET) {
    ...
    if ($yon == "ileri") {
        $sorgu = $baglanti->query("SELECT * FROM makale1 where id > $id limit 1"); // Makale tablosundaki veri çekiyoruz.
    }
    if ($yon == "geri") {
        $sorgu = $baglanti->query("SELECT * FROM makale1 where id < $id ORDER  by id desc limit 1"); // Makale tablosundaki veri çekiyoruz.
    }


} else {
    //eğer sayfa ilk defa açılmışsa ilk değeri alıyoruz.
    $sorgu = $baglanti->query("SELECT * FROM makale1 limit 1");
...
<div class="card-body"> 
    ...
    <h1 class="card-title text-center">Felsefi Görüş</h1>
    <p class="card-text">
...
function yolla(){
window.location.href="felsefi_gorus.php?yon=ileri&id=<?php echo $sonuc["id"]; ?>"
}
